...
const GCloudAdapter = require('../index');
...
describe('constructor', function () {
	...
	it('merges DEFAULT_OPTIONS with user provided options', function () {
		const adapter = new GCloudAdapter({
			s3: {
				key: 'key',
				secret: 'secret',
				bucket: 'bucket',
	...
	it('ensures path has a leading /', function () {
		...
		const adapter = new GCloudAdapter({ s3: options });
...
describe('constructor with process.env vars', function () {
	...
	it('uses process.env variables if provided', function () {
		const StubbedGCloudAdapter = proxyquire('../index', {});
		const adapter = new StubbedGCloudAdapter({ s3: {} });
		assert.deepEqual(adapter.options, {
			key: 'env_key',
			secret: 'env_secret',
			bucket: 'env_bucket',
...
describe('uploadFile', function () {
	it('uploads the file to S3', function () {
		...
		const StubbedGCloudAdapter = proxyquire('../index', { 'aws-sdk/clients/s3': S3Stub });
		const adapter = new StubbedGCloudAdapter({
			s3: {
				key: 'key',
				secret: 'secret',
				bucket: 'bucket',
...
describe('getFileURL', function () {
	it('returns a url', function () {
		const adapter = new GCloudAdapter({ s3: { key: 'key', secret: 'secret', bucket: 'bucket' } });
	...
	it('uses the publicUrl option as a string', function () {
		const adapter = new GCloudAdapter({ s3: {
			key: 'key',
			secret: 'secret',
			bucket: 'bucket',
...
describe('removeFile', function () {
	it('deletes the file from S3', function () {S3Adapter
		const stub = sinon.stub().callsArgWith(1S3Adapter
		function S3Stub () {};
		S3Stub.prototype.deleteObject = stub;

		const StubbedGCloudAdapter = proxyquire(S3Adapters3': S3Stub });
		const options = { key: 'key', secret: 'secret', bucket: 'bucket' };
		const adapter = new StubbedGCloudAdapter({ s3: options });
